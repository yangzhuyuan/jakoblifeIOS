...
<script setup>
	...
	export default {
		...
		onShow() {
			let that = this
			//获取手机时区开始
			// const gmtOffset = getGMTTimezoneStrict() // GMT+08:00
			// const timeZoneId = getDeviceTimeZone() // Asia/Shanghai
			// console.log("onShow", gmtOffset, timeZoneId)
			//获取手机时区结束
		...
		methods: {
				...
				u16proBLE.setForwardNotifyHandler(async (res, bytes, parsedResult) => {
					try {
						...
						if (parsedResult && (res.serviceId === BPW6SERVICE || isBcService)) {
							const BPW6SN = that.deviceIdwatch6_SN || deviceSn
			...
			share_data_fat_scale(deviceSn, deviceId, fattimes, aaa) {
				...
				let data = {
					...
					measurementTs: this.UTCdatatime().timestampSec,
					measurementTimezone: timeFields.measurementTimezone,
				}
					...
					if (ppgresultslist.code === 200 && ppgresultslist.data.length > 0) {
						...
						for (let p = ppgresultslist.data.length - 1; p >= 0; p--) {
							...
							if (recordId === 1) {
								...
								this.chartDataPPG.series[2].data.push(ppgresultslist.data[p]
									.stressIndex);
								...
								this.chartDataPPG.series[3].data.push(ppgresultslist.data[p]
									.fatigueIndex);
								this.optsPPG.extra.markLine.data[0].value = 5
								this.optsPPG.extra.markLine.data[0].lineColor = "#FF6B6B"
								this.optsPPG.extra.markLine.data[0].showLabel = true
					...
					if (ppgresultslist.code === 200 && ppgresultslist.data.length > 0) {
						...
						dailyAverages.forEach(item => {
							...
							if (recordId === 0) {
								...
								// 疲劳指数
								const avgRiskScore = item.averagesumfatigueIndex.toFixed(
									1);
								...
								// 恢复指数
								const avgRiskScore = item.averagesumrecoveryIndex.toFixed(
									1);
						...
						uni.getNetworkType({
							success: async function(res) {
								...
								if (isBpAccompanyingHr) {
									...
									console.log('BPW1手动心率无邻近血压，延迟单独上报:', hrDelayWallKey)
									setTimeout(() => {
										uni.getNetworkType({
					...
					case "1d": { //ACC和PPG数据处理
						...
						switch (Status) {
							case "01":
									...
									if (watchtime2 <= 0) {
										...
										if (that.quotientPPG > 0 && that.dataBuffer && that.dataBuffer.length <
											...
											if (!that.yalixueyatype && (that.immediateEmotionMeasure || that
													.sleep_alertid === 1 || uni.getStorageSync("sendwatch") ===
													1)) {
												uni.showLoading({
													title: that.$t("测量中"),
													mask: true,
								...
								// 传输开始后保持测量中 loading，直到 Status02 传完再关
								if (!that.yalixueyatype && (that.sleep_alertid === 1 || that
										.immediateEmotionMeasure || uni.getStorageSync("sendwatch") === 1)) {
									uni.showLoading({
										title: that.$t("测量中"),
										mask: true,
									})
			...
			// BPW6 心率历史去重上传（旧→新串行，最新一条由 jakoblife_fat_scale22 单独上报）
			...
			uploadBPW1HrHistoryWithDeduplication(localHrRecords, deviceId, deviceSn) {
				...
				uniqueRecords.forEach(hr => {
					...
					that.Watch_Historical_data(uploadMac, '', '', hrVal, uploadSn, that.datatime(key))
				})
				if (skipped) {
			...
			uploadBPW1SpO2HistoryWithDeduplication(localSpO2Records, deviceId, deviceSn) {
				...
				(localSpO2Records || []).forEach(item => {
					...
					that.jakoblife_fat_scale3(uploadMac, item.spO2, uploadSn, "血氧", that.datatime(key))
				})
				if (skipped) {
			...
			BPW6paredata(data, deviceId, deviceSn) {
				...
				switch (data.type) {
					...
					case 'BPdata': {
						...
						// 实时血压：与血氧一致，上报 time 用手机本地当前时间（不用设备中国时区戳）
						// 历史：仍用设备测量时间戳 / 本地墙钟解析
						const uploadTs = (that.bpw6RealtimeBpPending && !that.bpw6BpHistorySyncing) ?
							that.resolveRealtimeUploadTimeSec() :
							(hasRecordTs ? recordTs : that.datatime(normalizedKey))
						...
						if (that.hasBpw6BpUploaded(normalizedKey) ||
							...
							console.log('BPW6血压本会话已实时处理，跳过重复上报:', normalizedKey, uploadTs)
							break;
						}
					...
					case 'pulse_empty':
						...
						setTimeout(() => {
							console.log('无脉诊数据或完毕:',
								uni.getStorageSync("Taking_pulse"));
							that.Taking_pulse = []
							if (that.$refs.pulseDiagnosisRef) {
								that.$refs.pulseDiagnosisRef.loadRealData()
							}
			...
			recoverBpw6PpgSessionUi(reason, opts = {}) {
				...
				// 先清手动会话标记，再恢复按钮（否则 setSleepAlertDisabled 会挡恢复）
				this.immediateEmotionMeasure = false
				this.sleep_alertid = 0
				this._bpw6EmotionMeasureStartedAt = 0
				this._bpw6EmotionPullOnShowTried = false
				...
				if (canToast) {
					...
					console.log('【BPW6】PPG会话收尾', reason)
				}
			},
			...
			async fetchBPW6PPGRawData(deviceSn, deviceId) {
				if (this.bpw6PpgReading) {
					console.log('【BPW6】PPG数据正在读取中，跳过重复请求')
					return
				}
				...
				this.showBPW6PpgCloudLoading()
				try {
					console.log('【BPW6】开始拉取PPG原始数据: 0x4C一次 + 0x4D按序')
			...
			jakoblife_fat_scale22(deviceId, shousuoye, shuzhangya, maibo, deviceSn, timess, opts) {
				...
				const timeFields = this.buildUploadTimeFields(timess)
				let data = {
					deviceSn: !deviceSn ? uni.getStorageSync("deviceSn") : deviceSn,
					mac: deviceId,
					deviceTypeId: "2",
					...
					if (res.code == 200) {
						...
						if (res.rows.length === 0) {
							...
							if (allElementsNot3000 && allElementsNot3001) {
								that.setSleepAlertDisabled(true)
							} else if (!that.isBpw6PpgButtonBusy()) {
								// 测量会话中勿重置，避免 onShow→queryDevices 冲掉置灰
								that.setSleepAlertDisabled(false)
							}
			...
			xueyaclicklink(type) {
				...
				let that = this
				console.log(that.devicetype, that.devicetypelist)
				...
				if (that.devicetype === 30001) {
					uni.writeBLECharacteristicValue({
						...
						success(BPW6success) {
							...
							console.log("血压指令发送成功", BPW6success)
						},
						fail(BPW6fail) {
							uni.showToast({
