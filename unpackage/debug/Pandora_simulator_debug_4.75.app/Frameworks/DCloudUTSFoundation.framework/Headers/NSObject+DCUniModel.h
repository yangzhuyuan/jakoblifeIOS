 ...
 * Implementations of `NSCoding`, `NSCopying`, `-hash` and `-isEqual:`.
 ...
 See `DCUniModel` protocol for custom methods.
 
 
 Sample Code:
    
     ********************** json convertor *********************
     @interface DCUniAuthor : NSObject
     @property (nonatomic, strong) NSString *name;
     @property (nonatomic, assign) NSDate *birthday;
     @end
     @implementation DCUniAuthor
     @end
 
     @interface DCUniBook : NSObject
     @property (nonatomic, copy) NSString *name;
     @property (nonatomic, assign) NSUInteger pages;
     @property (nonatomic, strong) DCUniAuthor *author;
     @end
     @implementation DCUniBook
     @end
    
     int main() {
         // create model from json
         DCUniBook *book = [DCUniBook dcUTS_modelWithJSON:@"{\"name\": \"Harry Potter\", \"pages\": 256, \"author\": {\"name\": \"J.K.Rowling\", \"birthday\": \"1965-07-31\" }}"];
 
         // convert model to json
         NSString *json = [book dcUTS_modelToJSONString];
         // {"author":{"name":"J.K.Rowling","birthday":"1965-07-31T00:00:00+0000"},"name":"Harry Potter","pages":256}
     }
 
     ********************** Coding/Copying/hash/equal *********************
     @interface DCUniShadow :NSObject <NSCoding, NSCopying>
     @property (nonatomic, copy) NSString *name;
     @property (nonatomic, assign) CGSize size;
     @end
 
     @implementation DCUniShadow
     - (void)encodeWithCoder:(NSCoder *)aCoder { [self dcUTS_modelEncodeWithCoder:aCoder]; }
     - (id)initWithCoder:(NSCoder *)aDecoder { self = [super init]; return [self dcUTS_modelInitWithCoder:aDecoder]; }
     - (id)copyWithZone:(NSZone *)zone { return [self dcUTS_modelCopy]; }
     - (NSUInteger)hash { return [self dcUTS_modelHash]; }
     - (BOOL)isEqual:(id)object { return [self dcUTS_modelIsEqual:object]; }
     @end
 
 */
@interface NSObject (DCUniModel)


/// 获取属性值
/// @param propertyName 属性名称
- (id)dcUTS_getPropertyValue:(NSString *)propertyName;

/// 获取所有属性名称
- (NSArray *)dcUTS_propertiesForPropertyDescriptors;
 ...
 */
+ (nullable instancetype)dcUTS_modelWithJSON:(id)json;
...
/**
 ...
 property, this method will try to convert the value based on these rules:
     ...
     `NSString` -> SEL, Class.
 */
+ (nullable instancetype)dcUTS_modelWithDictionary:(NSDictionary *)dictionary;
 ...
 */
- (BOOL)dcUTS_modelSetWithJSON:(id)json;
...
/**
 ...
 @return Whether succeed.
 */
- (BOOL)dcUTS_modelSetWithDictionary:(NSDictionary *)dic;
 ...
 */
- (nullable id)dcUTS_modelToJSONObject;
 ...
 */
- (nullable NSData *)dcUTS_modelToJSONData;
...
/**
 ...
 inner object to json string.
 */
- (nullable NSString *)dcUTS_modelToJSONString;
 ...
 */
- (void)dcUTS_modelEncodeWithCoder:(NSCoder *)aCoder;
...
/**
 ...
 @return self
 */
- (id)dcUTS_modelInitWithCoder:(NSCoder *)aDecoder;
 ...
 */
- (BOOL)dcUTS_modelIsEqual:(id)model;
 ...
 */
- (NSString *)dcUTS_modelDescription;


/// 获取属性类型
/// @param name 属性名称
- (DCUniEncodingType)dcUTS_getModelPropertyTypeWithName:(NSString *)name;


/// 将 value转换为 NSNumber
/// @param value 数据
+ (NSNumber * __nullable)numberFromValue:(id)value;
 ...
 */
+ (nullable NSArray *)dcUTS_modelArrayWithClass:(Class)cls json:(id)json;
 ...
 */
@interface NSDictionary (DCUniModel)
 ...
 */
+ (nullable NSDictionary *)dcUTS_modelDictionaryWithClass:(Class)cls json:(id)json;
@end
...
/**
 ...
 There's no need to add '<DCUniModel>' to your class header.
 */
@protocol DCUniModel <NSObject>
@optional
...
/**
 ...
 object will be add to the array/set/dictionary.
  ...
  Example:
        @class DCUniShadow, DCUniBorder, DCUniAttachment;
 
        @interface DCUniAttributes
        @property NSString *name;
        ...
        @implementation DCUniAttributes
        + (NSDictionary *)modelContainerPropertyGenericClass {
            return @{@"shadows" : [DCUniShadow class],
                     @"borders" : DCUniBorder.class,
                     @"attachments" : @"DCUniAttachment" };
        }
        @end
...
/**
 ...
 Example:
        @class DCUniCircle, DCUniRectangle, DCUniLine;
 
        @implementation DCUniShape

        + (Class)modelCustomClassForDictionary:(NSDictionary*)dictionary {
            if (dictionary[@"radius"] != nil) {
                return [DCUniCircle class];
            } else if (dictionary[@"width"] != nil) {
                return [DCUniRectangle class];
            } else if (dictionary[@"y2"] != nil) {
                return [DCUniLine class];
            } else {
                return [self class];
            }
        }
