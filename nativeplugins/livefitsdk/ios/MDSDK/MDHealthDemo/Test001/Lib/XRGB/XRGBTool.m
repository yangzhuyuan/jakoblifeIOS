...
+ (uLong)getCRC32:(NSData *)data {
    return  crc32111(0xffffffff, data.bytes, (unsigned int)data.length);
}
...
+ (NSArray *)getRGBsArrFromImage:(UIImage *)image{
    ...
    for (int y = 0; y < imageH; y++) {
        for (int x = 0; x < imageW; x++) {
            ...
            pixelItem.red = red;
            pixelItem.green = green;
            pixelItem.blue = blue;
            pixelItem.alpha = alpha;
...
//            NSLog(@"RGB:%02X, %02X, %02X, %02X", red, green, blue, alpha);
