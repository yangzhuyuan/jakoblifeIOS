<template>
	<view class="fold-calendar">
		<!-- 头部上下月按钮及月份 -->
		<view class="header">
			<view class="header-item-box">
				<view class="left-ico-box" v-if="showChangeBtn" @click="changeMonthOrWeek('prev')">
					<image src="data:image/png;base64," mode=""></image>
				</view>
				<view class="header-time-box">
					{{ selDate.year + '年' + (selDate.month < 10 ? '0' + selDate.month : selDate.month) + '月' }}
				</view>
				<view class="left-ico-box" v-if="showChangeBtn" @click="changeMonthOrWeek('next')">
					<image src="data:image/png;base64," mode=""></image>
				</view>
			</view>
		...
		<swiper
			v-if="swiper"
			class="calendar-data"
			:current="shrinkType ? tranCurrent : tranIndex"
			circular
			:duration="tranDuration"
			@change="swiperChange"
			@animationfinish="swiperEndChange"
			:style="{ height: shrinkType ? '112rpx' : '532rpx' }"
		>
					...
					<view class="week-days" v-for="(b, j) in a" :key="j">
						<view class="day" v-for="(c, k) in b" :key="k" @click="clickDay(c)">
							<view class="day-info" :class="[c.dayClass, getIsSelDay(c) && 'is-sel', c.dayType != 'normal' && 'un-month']">
								<text class="day-solar" v-if="c.isToday">今天</text>
		...
		<swiper
			...
			:style="{ height: shrinkType ? '112rpx' : '532rpx' }"
		>
			<swiper-item class="swiper-item swiper-prev-item" v-for="(a, i) in getAllData" :key="i" catchtouchmove="stopTouchMove">
				<text class="month-bg" v-if="showMonthBg">{{ getMontBg }}</text>
				<view class="month-days" :class="[shrinkType ? 'item-week' : '']">
					<view class="week-days" v-for="(b, j) in a" :key="j">
						<view class="day" v-for="(c, k) in b" :key="k" @click="clickDay(c)">
		...
		<view class="shrink" v-if="showShrink" @click="changeShrink">
			...
			<view class="shrink-btn-box">
				<view class="shrink-btn" :class="[shrinkType ? 'shrink-open' : 'shrink-close']"></view>
			</view>
		</view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		getDayInfo(date, dayType) {
			...
			let holidayArr = ['元旦', '春节', '清明', '劳动节', '端午节', '中秋节', '国庆节'];
			let isHoliday = false;
